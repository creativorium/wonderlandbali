...
// The badges are shown at ~96px but the sources are 341KB and 421KB PNGs.
if ( function_exists( 'wonderland_bg_url' ) ) {
	$badge_1 = wonderland_bg_url( $badge_1, 'medium' );
	$badge_2 = wonderland_bg_url( $badge_2, 'medium' );
}
...
<section class="site-press" aria-label="<?php esc_attr_e( 'Featured in', 'wonderland' ); ?>">
	<div class="site-press__track">
		...
		<?php for ( $run = 0; $run < 2; $run++ ) : ?>
			<?php for ( $i = 1; $i <= 6; $i++ ) : ?>
				<?php
				// Through the media helper when the blocks plugin is present, so the
				// marquee gets the WebP derivative instead of the source PNG — these
				// were up to 271KB each, on every page.
				$press_src = $uploads . '/2023/06/' . $i . '.png';
				if ( function_exists( 'wonderland_bg_url' ) ) {
					$press_src = wonderland_bg_url( $press_src, 'medium' );
				}
				?>
				<img class="site-press__logo" src="<?php echo esc_url( $press_src ); ?>" alt="" loading="lazy" decoding="async" <?php echo $run ? 'aria-hidden="true"' : ''; ?> />
			<?php endfor; ?>
		<?php endfor; ?>
	</div>
</section>
